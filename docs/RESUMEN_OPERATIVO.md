...
| Integraciones | Google Sheets CSV / Drive API; Buk RRHH + Buk Asistencia; IMAP (PDFs arqueo) |
...
3. `pip install -r requirements.txt` si cambiaron deps.
4. Ejecutar bloques nuevos de `QUERY_CAMBIOS_PRODUCCION.sql` si aplica.
5. Reiniciar app.
6. Smoke test: login, dashboard/ventas, un módulo tocado, `/refresh` solo con sesión.
