...
5. **Puntos sin ruta:** `POST …/ruta/puntos-google` — genera KML temporal y abre **todos los pins juntos** en Google Maps (una ventana, sin trazar camino). `GET …/ruta/kml/<token>.kml` sirve el archivo (público, token aleatorio). **Mapa solo puntos** (`POST …/ruta/puntos-mapa`) — mapa embebido Leaflet/OSM (requiere ORS).
